#!/usr/bin/env sh
set -e
set -u

if [ -t 1 ]; then
	printf '%s\n' 'This script will install the apt repository fatten' 'It will change your apt keys, create or replace /etc/apt/sources.list.d/00fatten.sources.list, install apt-transport-https and update apt.' 'Press the [Enter] key to continue.'
	read -r garbage
fi

sudo -p "Password for %p to allow root to update from new sources before installing apt-transport-https: " apt-get --quiet update
sudo -p "Password for %p to allow root to  apt-get install apt-transport-https (missing in Debian default installs)" apt-get install apt-transport-https

temporaryKeyFile="$(mktemp --tmpdir fatten.key.XXXXXXXXX)"
trap 'rm -rf "$temporaryKeyFile"' EXIT HUP INT QUIT TERM
cat >"$temporaryKeyFile" <<EOF
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1


=2vnB
-----END PGP PUBLIC KEY BLOCK-----
EOF
sudo -p "Password for %p is required to allow root to install repository 'fatten' public key to apt: " apt-key add "$temporaryKeyFile"

echo 'deb https://shellfire-dev.github.io/fatten/download/apt fatten multiverse' | sudo -p "Password for %p is required to allow root to install repository 'fatten' apt sources list to '/etc/apt/sources.list.d/00fatten.sources.list': " tee /etc/apt/sources.list.d/00fatten.list >/dev/null
sudo -p "Password for %p to allow root to update from new sources: " apt-get --quiet update
